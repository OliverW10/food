...
export default function PostPage() {
    ...
    const [showComments, setShowComments] = useState(false);
        ...
        const { data, isLoading, isError, error } = trpc.post.getById.useQuery({ id: id ?? -1 }, {
            enabled: !!id,
            staleTime: 0,
    ...
    if (isError || !data) {
        return <Text style={{ color: "#fff" }}>{error instanceof Error ? error.message : "Post not found"}</Text>;
    }
        ...
        const post = {
            ...
            author: {
                id: data.author?.id ?? 0,
                name: data.author?.name ?? (data.author?.email ? data.author.email.split("@")[0] : ""),
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: "#0b0f16" }}>
            <View style={{ flex: 1, justifyContent: "center", alignItems: "center", padding: 16 }}>
                <FoodPost review={post} onOpenComments={() => setShowComments(true)} style={{ width: "100%" }} />
            </View>
            <CommentsSheet postId={post.id} visible={showComments} onClose={() => setShowComments(false)} />
        </SafeAreaView>
    );
}