...
export default function FollowersPage() {
  ...
  const followMutation = trpc.search.followToggle.useMutation({
    onMutate: async ({ targetUserId, follow }) => {
      ...
      const key = { limit: 20 };
      const previous = utils.search.followers.getInfiniteData(key);
      utils.search.followers.setInfiniteData(key, (cur) => {
        if (!cur) return cur;
        const next = {
    ...
    onError: (_e, _v, ctx) => {
      if (ctx?.previous)
        utils.search.followers.setInfiniteData(ctx.key, ctx.previous);
    },
    onSettled: async (_d, _e, vars) => {
      await utils.search.followers.invalidate({ limit: 20 });
      try {
        await utils.profile?.get?.invalidate({ id: vars.targetUserId } as any);
      } catch {}
      try {
        await utils.profile?.get?.invalidate();
      } catch {}
      try {
        await utils.post?.getFeed?.invalidate({ mode: "following" } as any);
      } catch {}
    },
  });
