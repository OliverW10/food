...
export const VersionInfoComponent = () => {
  ...
  return (
      <>
        ...
        {versionInfo.missingInDb.length ? <Text>Missing migrations in DB (server is ahead of database): {versionInfo.missingInDb.join(", ")}</Text> : ""}
        {versionInfo.missingInFiles.length ? <Text>Missing migrations in server (database is ahead of server): {versionInfo.missingInFiles.join(", ")}</Text> : ""}
      </>
  )
}