...
export function CommentsSheet({
  ...
  const commentsListQ = (trpc as any).comments?.list?.useInfiniteQuery?.(
    input,
    { enabled: visible && !!postId, getNextPageParam: (last: any) => last?.nextCursor ?? null }
    ...
    <Modal animationType="slide" transparent visible={visible} onRequestClose={onClose}>
      <View style={{ flex:1, backgroundColor:'rgba(0,0,0,0.5)', justifyContent:'flex-end' }}>
        <View style={{
          ...
          <View style={{ alignSelf:'center', width:40, height:4, backgroundColor:'#374151', borderRadius:4, marginBottom:8 }} />
          <Text style={{ fontSize:18, fontWeight:'700', marginBottom:12, color:'#fff' }}>Comments</Text>
          ...
          {commentsListQ.isLoading ? (
            ...
            <ScrollView>
              {items.map((c: any) => (
                <View key={c.id} style={{ paddingVertical:8, borderBottomWidth:1, borderColor:'#1f2937' }}>
          ...
          <View style={{ marginTop:12 }}>
            ...
            />
            <TouchableOpacity
              style={{ padding:12, backgroundColor:'#2563eb', borderRadius:8, alignItems:'center' }}
              onPress={() => postId && text.trim() && addMutation.mutate({ postId, text: text.trim() })}
            >
              <Text style={{ color:'#fff', fontWeight:'600' }}>Post</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={{ marginTop:8, padding:12, backgroundColor:'#374151', borderRadius:8, alignItems:'center' }}
              onPress={onClose}
            >
